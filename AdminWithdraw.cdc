...
transaction(senderAccount: Address, amount: UFix64) {
    ...
    // Define references
    let senderVault: &NftToken.Vault{NftToken.VaultInterface}
    ...
    let adminResource: &NftToken.Admin
    let flowMinter: &FlowToken.Minter

    prepare(acct: AuthAccount) {
        // Borrow references and handle errors
        self.adminResource = acct.borrow<&NftToken.Admin>(from: /storage/AdminStorage)
        ...
        self.senderVault = getAccount(senderAccount)
            ...
            .borrow<&NftToken.Vault{NftToken.VaultInterface}>()
