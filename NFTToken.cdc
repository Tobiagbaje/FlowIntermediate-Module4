...
// NftToken contract: Implements a fungible token on the Flow blockchain
pub contract NftToken: FungibleToken {

    // Total supply of NftToken
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, VaultInterface {
        ...
        pub fun deposit(from: @FungibleToken.Vault) {
            let vault <- from as! @NftToken.Vault
        ...
        // Destroy the vault
        destroy() {
            NftToken.totalSupply = NftToken.totalSupply - self.balance
        }
    }
    ...
    pub resource Minter {
        ...
        pub fun mintToken(amount: UFix64): @FungibleToken.Vault {
            NftToken.totalSupply = NftToken.totalSupply + amount
            return <-create Vault(balance: amount)
        }
    }
    ...
    init() {
        ...
        self.account.link<&NftToken.Minter>(/public/Minter, target: /storage/MinterStorage)
